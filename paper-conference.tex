...
As hardware designs become more complex, and the allure of hardware acceleration on platforms like field-programmable gate arrays (FPGAs) becomes more attractive to corporations and others seeking to maximize the performance of their algorithms, it becomes necessary for tools to be developed that allow for more effective use of hardware acceleration. One of the tools used in this regard is Vitis HLS, Xilinx's proprietary high-level synthesis C++ tool, which uses C++ code along with a few simple preprocessor directives to created hardware designs on a higher, more conceptual level. In order to better facilitate high-level synthesis of hardware designs, Xilinx developed an additional flow target for the Vitis HLS tool called the Vitis Kernel Flow.
Our goal was to explore this new flow, and compare its performance with the older Vivado IP Flow, implementing the \textit{k}-nearest neighbors algorithm for image recognition, using the Xilinx Alveo U280 server FPGA board with high-bandwidth memory.
...
In our pure software approach, we used a MicroBlaze soft processor to control the board. Unfortunately, the size of the C++ libraries we needed to include to run our existing code meant a major change to many of our functions, types, and arrays. Eventually, we ran into a wall and we were unable to get a pure software approach that only used the MicroBlaze to work. We then moved on to the next method, in order to have bit more memory to work with.

\newpage

\subsubsection{Software + DRAM}

By adding the DRAM to the design, we were able to get the space we needed to hold the C++ libraries and to hold the training and test data for the algorithm.

\subsubsection{Software + HBM}

We were not able to get HBM to work.
...
As above, we were not able to get HBM to work.
...
In conclusion, the Vitis Kernel Flow has documentation, but it is difficult to find and understand for the new user. Additionally, the hardware accelerated algorithm implemented purely in lookup tables and flip flops ran faster that the Microblaze processor and hardware algorithm implemented with DRAM.
